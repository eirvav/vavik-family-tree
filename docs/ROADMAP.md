...
| 4b-polish-2. Layout overlap fix + straight edges + combined parents | ⬜ Plan ready, not started | Recursive subtree-width Dagre layout (fixes a confirmed node-overlap bug); straight partner/sibling edges matching a reference layout (no direct sibling line when a parent is shared); single "Legg til foreldre" action replacing separate add-father/add-mother (creates both, already connected). Spec and plan are written and committed; execution hasn't started — next session should run subagent-driven-development against the plan below. |
